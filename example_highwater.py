...
async def hg():
	while True:
		...
		states = consumer._subscription.subscription.assignment._tp_state
		print (['%s: %s-%s-%s' % (tp.partition, state._position, state.highwater, state.lso) for tp, state in states.items()])


loop.run_until_complete(asyncio.wait([check(), hg()]))
